...
'use strict';
angular.module('restaurantApp').controller('userDetailCtrl', ['$scope', '$state', '$http', 'Data', '$location', '$stateParams', 'Notification', function ($scope, $state, $http, Data, $location, $stateParams, Notification) {
        $scope.formData = {};
        ...
        var initialize = function () {
            if ($stateParams.id) {
                Data.getUserData({user_id: $stateParams.id}, function (result) {
                    ...
                    $scope.formData = data;
                    // $scope.formData.mobile_no= parseInt(data.mobile_no);
